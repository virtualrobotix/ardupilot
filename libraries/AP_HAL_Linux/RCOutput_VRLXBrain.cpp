...
#define PCA9685_ADDRESS            0x40 // All address pins low, VRLXBrain default
 ...
 */
#define PCA9685_INTERNAL_CLOCK 24576000.f
...
LinuxRCOutput_VRLXBrain::LinuxRCOutput_VRLXBrain():
    _i2c_sem(NULL),
    enable_pin(NULL),
    _frequency(50),
...
void LinuxRCOutput_VRLXBrain::init(void* machtnicht)
{
    _i2c_sem = hal.i2c->get_semaphore();
    if (_i2c_sem == NULL) {
        hal.scheduler->panic(PSTR("PANIC: RCOutput_VRLXBrain did not get "
                                  "valid I2C semaphore!"));
...
void LinuxRCOutput_VRLXBrain::reset_all_channels()
{
    if (!_i2c_sem->take(10)) {
        return;
    }
...
void LinuxRCOutput_VRLXBrain::set_freq(uint32_t chmask, uint16_t freq_hz)
{
    ...
    uint8_t prescale = round(PCA9685_INTERNAL_CLOCK / 4096.f / freq_hz)  - 1;
...
uint16_t LinuxRCOutput_VRLXBrain::get_freq(uint8_t ch)
{
    return _frequency;
}

void LinuxRCOutput_VRLXBrain::enable_ch(uint8_t ch)
{

}

void LinuxRCOutput_VRLXBrain::disable_ch(uint8_t ch)
{
    write(ch, 0);
}

void LinuxRCOutput_VRLXBrain::write(uint8_t ch, uint16_t period_us)
{
...
void LinuxRCOutput_VRLXBrain::write(uint8_t ch, uint16_t* period_us, uint8_t len)
{
    for (int i = 0; i < len; i++)
        write(ch + i, period_us[i]);
}

uint16_t LinuxRCOutput_VRLXBrain::read(uint8_t ch)
{
    return _pulses_buffer[ch];
}

void LinuxRCOutput_VRLXBrain::read(uint16_t* period_us, uint8_t len)
{
    for (int i = 0; i < len; i++) 
