...
#include <sys/mman.h>
#include <assert.h>
#include "RCInput_VRLXBrain.h"

//Parametres
#define RCIN_VRLXBRAIN_BUFFER_LENGTH   8
#define RCIN_VRLXBRAIN_SAMPLE_FREQ     500
#define RCIN_VRLXBRAIN_DMA_CHANNEL     0
#define RCIN_VRLXBRAIN_MAX_COUNTER     1300
#define PPM_INPUT_VRLXBRAIN RPI_GPIO_4
#define RCIN_VRLXBRAIN_MAX_SIZE_LINE   50

//Memory Addresses
#define RCIN_VRLXBRAIN_RPI1_DMA_BASE 0x20007000
#define RCIN_VRLXBRAIN_RPI1_CLK_BASE 0x20101000
#define RCIN_VRLXBRAIN_RPI1_PCM_BASE 0x20203000

#define RCIN_VRLXBRAIN_RPI2_DMA_BASE 0x3F007000
#define RCIN_VRLXBRAIN_RPI2_CLK_BASE 0x3F101000
#define RCIN_VRLXBRAIN_RPI2_PCM_BASE 0x3F203000

#define RCIN_VRLXBRAIN_GPIO_LEV0_ADDR  0x7e200034
#define RCIN_VRLXBRAIN_DMA_LEN         0x1000
#define RCIN_VRLXBRAIN_CLK_LEN         0xA8
#define RCIN_VRLXBRAIN_PCM_LEN         0x24
#define RCIN_VRLXBRAIN_TIMER_BASE      0x7e003004

#define RCIN_VRLXBRAIN_DMA_SRC_INC     (1<<8)
#define RCIN_VRLXBRAIN_DMA_DEST_INC    (1<<4)
#define RCIN_VRLXBRAIN_DMA_NO_WIDE_BURSTS  (1<<26)
#define RCIN_VRLXBRAIN_DMA_WAIT_RESP   (1<<3)
#define RCIN_VRLXBRAIN_DMA_D_DREQ      (1<<6)
#define RCIN_VRLXBRAIN_DMA_PER_MAP(x)  ((x)<<16)
#define RCIN_VRLXBRAIN_DMA_END         (1<<1)
#define RCIN_VRLXBRAIN_DMA_RESET       (1<<31)
#define RCIN_VRLXBRAIN_DMA_INT         (1<<2)

#define RCIN_VRLXBRAIN_DMA_CS          (0x00/4)
#define RCIN_VRLXBRAIN_DMA_CONBLK_AD   (0x04/4)
#define RCIN_VRLXBRAIN_DMA_DEBUG       (0x20/4)

#define RCIN_VRLXBRAIN_PCM_CS_A        (0x00/4)
#define RCIN_VRLXBRAIN_PCM_FIFO_A      (0x04/4)
#define RCIN_VRLXBRAIN_PCM_MODE_A      (0x08/4)
#define RCIN_VRLXBRAIN_PCM_RXC_A       (0x0c/4)
#define RCIN_VRLXBRAIN_PCM_TXC_A       (0x10/4)
#define RCIN_VRLXBRAIN_PCM_DREQ_A      (0x14/4)
#define RCIN_VRLXBRAIN_PCM_INTEN_A     (0x18/4)
#define RCIN_VRLXBRAIN_PCM_INT_STC_A   (0x1c/4)
#define RCIN_VRLXBRAIN_PCM_GRAY        (0x20/4)

#define RCIN_VRLXBRAIN_PCMCLK_CNTL     38
#define RCIN_VRLXBRAIN_PCMCLK_DIV      39
...
volatile uint32_t *LinuxRCInput_VRLXBrain::pcm_reg;
volatile uint32_t *LinuxRCInput_VRLXBrain::clk_reg;
volatile uint32_t *LinuxRCInput_VRLXBrain::dma_reg;
...
{
    char buffer[RCIN_VRLXBRAIN_MAX_SIZE_LINE];
    ...
    while (fgets(buffer, RCIN_VRLXBRAIN_MAX_SIZE_LINE, fd) != NULL) {
...
void LinuxRCInput_VRLXBrain::set_physical_addresses(int version)
{
    if (version == 1) {
        dma_base = RCIN_VRLXBRAIN_RPI1_DMA_BASE;
        clk_base = RCIN_VRLXBRAIN_RPI1_CLK_BASE;
        pcm_base = RCIN_VRLXBRAIN_RPI1_PCM_BASE;
    }                                                                                                                                     
    else if (version == 2) {
        dma_base = RCIN_VRLXBRAIN_RPI2_DMA_BASE;
        clk_base = RCIN_VRLXBRAIN_RPI2_CLK_BASE;
        pcm_base = RCIN_VRLXBRAIN_RPI2_PCM_BASE;
    }
}

//Map peripheral to virtual memory
void* LinuxRCInput_VRLXBrain::map_peripheral(uint32_t base, uint32_t len)
{
...
void LinuxRCInput_VRLXBrain::init_dma_cb(dma_cb_t** cbp, uint32_t mode, uint32_t source, uint32_t dest, uint32_t length, uint32_t stride, uint32_t next_cb)
{
    (*cbp)->info = mode;
    (*cbp)->src = source;
    (*cbp)->dst = dest;
...
void LinuxRCInput_VRLXBrain::stop_dma()
{
    dma_reg[RCIN_VRLXBRAIN_DMA_CS | RCIN_VRLXBRAIN_DMA_CHANNEL << 8] = 0;
}

/* We need to be sure that the DMA is stopped upon termination */
void LinuxRCInput_VRLXBrain::termination_handler(int signum)
{
    stop_dma();
...
void LinuxRCInput_VRLXBrain::init_ctrl_data()
{
  ...
  uint32_t i = 0;
  for (i = 0; i < 56 * 128 * RCIN_VRLXBRAIN_BUFFER_LENGTH; i++) // 8 * 56 * 128 == 57344
	{
      //Transfer timer every 56th sample
      if(i % 56 == 0) {
          cbp_curr = (dma_cb_t*)con_blocks->get_page(con_blocks->_virt_pages, cbp);

          init_dma_cb(&cbp_curr, RCIN_VRLXBRAIN_DMA_NO_WIDE_BURSTS | RCIN_VRLXBRAIN_DMA_WAIT_RESP | RCIN_VRLXBRAIN_DMA_DEST_INC | RCIN_VRLXBRAIN_DMA_SRC_INC, RCIN_VRLXBRAIN_TIMER_BASE, (uint32_t) circle_buffer->get_page(circle_buffer->_phys_pages, dest), 8, 0, (uint32_t) con_blocks->get_page(con_blocks->_phys_pages, cbp + sizeof(dma_cb_t)));
          dest += 8;
          cbp += sizeof(dma_cb_t);
      } 
	    ...
	    cbp_curr = (dma_cb_t*)con_blocks->get_page(con_blocks->_virt_pages, cbp);
	    init_dma_cb(&cbp_curr, RCIN_VRLXBRAIN_DMA_NO_WIDE_BURSTS | RCIN_VRLXBRAIN_DMA_WAIT_RESP, RCIN_VRLXBRAIN_GPIO_LEV0_ADDR, (uint32_t) circle_buffer->get_page(circle_buffer->_phys_pages, dest), 1, 0, (uint32_t) con_blocks->get_page(con_blocks->_phys_pages, cbp + sizeof(dma_cb_t)));
	    dest += 1;
	    cbp += sizeof(dma_cb_t);	  
	    ...
	    */
	    cbp_curr = (dma_cb_t*)con_blocks->get_page(con_blocks->_virt_pages, cbp);
	    init_dma_cb(&cbp_curr, RCIN_VRLXBRAIN_DMA_NO_WIDE_BURSTS | RCIN_VRLXBRAIN_DMA_WAIT_RESP | RCIN_VRLXBRAIN_DMA_D_DREQ | RCIN_VRLXBRAIN_DMA_PER_MAP(2), RCIN_VRLXBRAIN_TIMER_BASE, phys_fifo_addr, 4, 0, (uint32_t)  con_blocks->get_page(con_blocks->_phys_pages, cbp + sizeof(dma_cb_t)));
	    cbp += sizeof(dma_cb_t);
	}
...
*/  
void LinuxRCInput_VRLXBrain::init_PCM()
{
    pcm_reg[RCIN_VRLXBRAIN_PCM_CS_A] = 1;                                          // Disable Rx+Tx, Enable PCM block
    hal.scheduler->delay_microseconds(100);
    clk_reg[RCIN_VRLXBRAIN_PCMCLK_CNTL] = 0x5A000006;                              // Source=PLLD (500MHz)
    hal.scheduler->delay_microseconds(100);
    clk_reg[RCIN_VRLXBRAIN_PCMCLK_DIV] = 0x5A000000 | ((50000/RCIN_VRLXBRAIN_SAMPLE_FREQ)<<12);   // Set pcm div. If we need to configure DMA frequency.
    hal.scheduler->delay_microseconds(100);
    clk_reg[RCIN_VRLXBRAIN_PCMCLK_CNTL] = 0x5A000016;                              // Source=PLLD and enable
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_VRLXBRAIN_PCM_TXC_A] = 0<<31 | 1<<30 | 0<<20 | 0<<16;             // 1 channel, 8 bits
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_VRLXBRAIN_PCM_MODE_A] = (10 - 1) << 10;                           //PCM mode
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_VRLXBRAIN_PCM_CS_A] |= 1<<4 | 1<<3;                               // Clear FIFOs
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_VRLXBRAIN_PCM_DREQ_A] = 64<<24 | 64<<8;                           // DMA Req when one slot is free?
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_VRLXBRAIN_PCM_CS_A] |= 1<<9;                                      // Enable DMA
    hal.scheduler->delay_microseconds(100);
    pcm_reg[RCIN_VRLXBRAIN_PCM_CS_A] |= 1<<2;                                      // Enable Tx
    hal.scheduler->delay_microseconds(100);
}
...
/*Initialise DMA
  ...
  http://www.raspberrypi.org/wp-content/uploads/2012/02/BCM2835-ARM-Peripherals.pdf
*/  
void LinuxRCInput_VRLXBrain::init_DMA()
{
    dma_reg[RCIN_VRLXBRAIN_DMA_CS | RCIN_VRLXBRAIN_DMA_CHANNEL << 8] = RCIN_VRLXBRAIN_DMA_RESET;                 //Reset DMA
    hal.scheduler->delay_microseconds(100);
    dma_reg[RCIN_VRLXBRAIN_DMA_CS | RCIN_VRLXBRAIN_DMA_CHANNEL << 8] = RCIN_VRLXBRAIN_DMA_INT | RCIN_VRLXBRAIN_DMA_END;
    dma_reg[RCIN_VRLXBRAIN_DMA_CONBLK_AD | RCIN_VRLXBRAIN_DMA_CHANNEL << 8] = reinterpret_cast<uint32_t>(con_blocks->get_page(con_blocks->_phys_pages, 0));//Set first control block address
    dma_reg[RCIN_VRLXBRAIN_DMA_DEBUG | RCIN_VRLXBRAIN_DMA_CHANNEL << 8] = 7;                      // clear debug error flags
    dma_reg[RCIN_VRLXBRAIN_DMA_CS | RCIN_VRLXBRAIN_DMA_CHANNEL << 8] = 0x10880001;                // go, mid priority, wait for outstanding writes
}


//We must stop DMA when the process is killed
void LinuxRCInput_VRLXBrain::set_sigaction()
{
    for (int i = 0; i < 64; i++) { 
        //catch all signals (like ctrl+c, ctrl+z, ...) to ensure DMA is disabled
        struct sigaction sa;
        memset(&sa, 0, sizeof(sa));
        sa.sa_handler = LinuxRCInput_VRLXBrain::termination_handler;
        sigaction(i, &sa, NULL);
    }
}

//Initial setup of variables
LinuxRCInput_VRLXBrain::LinuxRCInput_VRLXBrain():
    prev_tick(0),
    delta_time(0),
    curr_tick_inc(1000/RCIN_VRLXBRAIN_SAMPLE_FREQ),
    curr_pointer(0),
    curr_channel(0),
    width_s0(0),
    curr_signal(0),
    last_signal(228),
    state(RCIN_VRLXBRAIN_INITIAL_STATE)
{
    ...
    circle_buffer = new Memory_table(RCIN_VRLXBRAIN_BUFFER_LENGTH * 2, version);
    con_blocks = new Memory_table(RCIN_VRLXBRAIN_BUFFER_LENGTH * 113, version);
}
...
void LinuxRCInput_VRLXBrain::deinit()
{
    stop_dma();
}

//Initializing necessary registers
void LinuxRCInput_VRLXBrain::init_registers()
{
    dma_reg = (uint32_t*)map_peripheral(dma_base, RCIN_VRLXBRAIN_DMA_LEN);
    pcm_reg = (uint32_t*)map_peripheral(pcm_base, RCIN_VRLXBRAIN_PCM_LEN);
    clk_reg = (uint32_t*)map_peripheral(clk_base, RCIN_VRLXBRAIN_CLK_LEN);
}

void LinuxRCInput_VRLXBrain::init(void*)
{
    
    init_registers();
    
    //Enable PPM input
    enable_pin = hal.gpio->channel(PPM_INPUT_VRLXBRAIN);
...
//Processing signal
void LinuxRCInput_VRLXBrain::_timer_tick()
{
    int j;
    void* x;

    //Now we are getting address in which DMAC is writing at current moment
    dma_cb_t* ad = (dma_cb_t*) con_blocks->get_virt_addr(dma_reg[RCIN_VRLXBRAIN_DMA_CONBLK_AD | RCIN_VRLXBRAIN_DMA_CHANNEL << 8]);
    for(j = 1; j >= -1; j--){
    ...
    if (counter > RCIN_VRLXBRAIN_MAX_COUNTER) {
        counter = RCIN_VRLXBRAIN_MAX_COUNTER;
    }
    ...
    for (;counter > 0x40;counter--) {
        ...
        if (curr_signal != last_signal) {
            ...
            prev_tick = curr_tick;
            switch (state) {
            case RCIN_VRLXBRAIN_INITIAL_STATE:
                state = RCIN_VRLXBRAIN_ZERO_STATE;
                break;
            case RCIN_VRLXBRAIN_ZERO_STATE:
                if (curr_signal == 0) {
                    width_s0 = (uint16_t) delta_time;
                    state = RCIN_VRLXBRAIN_ONE_STATE;
                    break;
                }
                else 
                    break;
            case RCIN_VRLXBRAIN_ONE_STATE:
                if (curr_signal == 1) {
