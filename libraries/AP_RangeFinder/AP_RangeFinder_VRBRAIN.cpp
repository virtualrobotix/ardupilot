...
uint8_t AP_RangeFinder_VRBRAIN::num_px4_instances = 0;
...
*/
AP_RangeFinder_VRBRAIN::AP_RangeFinder_VRBRAIN(RangeFinder &_ranger, uint8_t instance, RangeFinder::RangeFinder_State &_state) :
    ...
    _last_min_distance_cm(-1)
{
	...
	if (_fd == -1) {
        hal.console->printf("Unable to open VRBRAIN rangefinder %u\n", num_px4_instances);
        state.healthy = false;
        return;
	}
...
*/
AP_RangeFinder_VRBRAIN::~AP_RangeFinder_VRBRAIN()
{
    if (_fd != -1) {
        close(_fd);
    }
...
/* 
   open the VRBRAIN driver, returning the file descriptor
*/
int AP_RangeFinder_VRBRAIN::open_driver(void)
{
    // work out the device path based on how many VRBRAIN drivers we have loaded
...
/* 
   see if the VRBRAIN driver is available
*/
bool AP_RangeFinder_VRBRAIN::detect(RangeFinder &_ranger, uint8_t instance)
{
    int fd = open_driver();
    if (fd == -1) {
        return false;
...
void AP_RangeFinder_VRBRAIN::update(void)
{
    if (_fd == -1) {
        state.healthy = false;
        return;
