...
static void handleTxInterrupt(uint8_t iface_index)
{
    if (iface_index < CAN_STM32_NUM_IFACES) {
        ...
        VRBRAINCAN* iface = ((VRBRAINCANManager*) hal.can_mgr)->getIface(iface_index);
        if (iface != nullptr) {
...
VRBRAINCAN* VRBRAINCANManager::ifaces[CAN_STM32_NUM_IFACES] = {};
    ...
    if (can_number <= CAN_STM32_NUM_IFACES) {
        ...
        if (can_number >= 1) {
            ...
            if (res < 0) {             // a typical race condition.
                printf("VRBRAINCANManager::init Iface 0 init failed %i\n\r", res);
                ifaces[0] = nullptr;
                return res;
            }
        }
        ...
        if (can_number >= 2) {
            ...
            if (res < 0) {
                printf("VRBRAINCANManager::init Iface 1 init failed %i\n\r", res);
                ifaces[1] = nullptr;
                return res;
            }
        }
